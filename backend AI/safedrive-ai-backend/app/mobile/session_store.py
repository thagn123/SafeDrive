    ...
    async def answer_assistant(self, request: AssistantQueryRequest) -> AssistantQueryResponse:
        ...
        async with self._lock:
            ...
            if session.state is None or session.last_update is None:
                ...
                default_signals = DriverSupportSignals(
                    steeringSignalAvailable=False,
                    seatSensorAvailable=False,
                    availableSourceCount=0,
                    totalSourceCount=0,
                )
                default_request = StateUpdateRequest(
                    sessionId=request.sessionId,
                    state=default_state,
                    driverSupportSignals=default_signals,
                    source="PHONE_SIMULATOR",
                    clientEventId=f"init-{request.requestId}",
                )
                snapshot = self._context_builder.build(
                    default_request,
                    state_version=1,
                ...
                evaluation = self._safety_engine.evaluate(snapshot, now_ms=started_at)
                session.state = StateEnvelope(
                    state=default_state,
                    driverSupportSignals=default_signals,
                    riskAssessment=evaluation.risk,
                    restRecommendation=evaluation.rest,
                    stateVersion=1,
                    acceptedAtMs=started_at,
                )
                session.last_update = default_request
            snapshot = self._context_builder.build(
                session.last_update,
                state_version=session.state.stateVersion,
                now_ms=started_at,
